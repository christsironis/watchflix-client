...
async function AddItems( page, obs, dispatch, limit=18) {
	...
	const res = await fetch( `http://localhost:3001/api/newSeries?page=${ page.current }&limit=${limit}` );
...
export async function getStaticProps() {
    const res = await fetch('http://localhost:3001/api/newSeries?page=1&limit=20');
    const json = await res.json();
    return {
      props: {
          json: json
